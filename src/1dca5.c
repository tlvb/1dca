...
#define WIDTH 1920
#define HEIGHT 1080
#define LAYERWIDTH 4096
#define TEXTURE_WIDTH LAYERWIDTH
#define TEXTURE_HEIGHT 2048
#define FRAMES 150
#define BASERATE 1000
#define RULE 184
#define SCALE 0
#define SCREEN_FLAGS SDL_WINDOW_FULLSCREEN
typedef struct { /*{{{*/
	int w;
	int h;
...
int main(void) { /*{{{*/
	...
	fprintf(stderr, " ok\n");/*}}}*/
	fprintf(stderr, "outputting base layers...");
	for (int i=0; i<LAYERS; ++i) {
		output_ppm(stdout, ls[0]->layer[i], ls[1]->layer[i], ls[2]->layer[i], ls[0]->w, ls[0]->h);
	}
