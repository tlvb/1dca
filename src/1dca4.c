...
void output_enumerated_png(const char *fmt, const char *dir, int i, uint8_t **layers, int w, int h) { /*{{{*/
	char namestr[2048];
	char cmdstr[2048];
	if (snprintf(namestr, 2048, fmt, dir, i) == 2047) {
...
int main(int argc, const char **argv) { /*{{{*/
	if (argc != 5) {
		fprintf(stderr, "run as %s outdir width height number-of-frames\n", argv[0]);
		return 1;
	}
	...
	for (int i=0; i<frames; ++i) {
		...
		fprintf(stderr, "!");
		output_enumerated_png("%s/rg_%06u.png", outdir, i, dst, width, height);
		fprintf(stderr, "]");
	}
	fprintf(stderr, " ok\n"); /*}}}*/
